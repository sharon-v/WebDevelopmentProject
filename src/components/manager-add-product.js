...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btn.addEventListener('click', (e) => {
      ...
      addProduct(Pname, description, price, sale, size90x200, size120x200, size160x200, size180x200, isJustLandedCbChecked,isOnSaleCbChecked, fabric, ImagesRef);

  });
});

function addProduct(Pname, description, price, sale, size90x200, size120x200, size160x200, size180x200, isJustLandedCbChecked,isOnSaleCbChecked, fabric, ImagesRef) {
  var sku;
  dbProducts.doc(Pname).set({
      sku: dbProducts.collection('...').get().then(snap => {
      ...
      isOnSaleCbChecked, isOnSaleCbChecked,
      fabric: fabric,
      ImagesRef:ImagesRef
    })
    .then(() => {
